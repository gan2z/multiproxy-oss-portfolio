...
**検証環境上で想定どおりに動作することを、スクリーンショットとログで確認する**ための検証ページです。
...
※本ページの技術スタック・ポート番号・ログは、個人のローカル検証環境における設定例です。
実在する企業・顧客・現場環境の構成を示すものではありません。
...
1. **主要コンポーネントが稼働し、構成図上の役割と対応付け可能**
...
**確認できること**
- 検証構成を構成する主要コンテナが running/healthy で稼働している（部分稼働ではない）
...
- “プロキシ3台を繋いだ” ではなく、**認証・検査・ログ・監視などの横断要素を含む検証構成**として成立している
...
### 4-0. 前提整理：本検証構成における SSLBump の適用方針」
...
- これは Squid の設定ミスではなく、  
  **本検証構成では、複数段で SSLBump を適用すると TLS セッションの扱いが複雑化し、通信が不安定になることを確認した**によるもの
...
- HTTPコード（結果）→ cache.log（原因）という <strong>運用を想定した切り分け思考</strong>を示せている
