...
本構成は「通常経路（Proxy1 経由）」と「DIRECT 経路（Client→Proxy2 直）」の 2 系統で構成しています。  
Proxy2 を分岐点とし、3129 を通常経路、3131 を DIRECT 経路として受け付けます。  
Proxy 間の通信は stunnel により TLS 中継し、ログはポート単位で分離しています。

通信経路を 2 系統に分離することで、  
業務通信（通常）と例外的・検証用途の通信（DIRECT）の責務を明確化し、  
障害時の切り分けやログ追跡を容易にする設計としています。  
DIRECT 経路は将来的に、仮想ブラウザ等を用いた高リスクサイト閲覧向け経路として拡張可能な位置づけです。
...
- Proxy2 → Proxy3（通常）：23129 → 4433 (TLS) → 3130  
- Proxy2 → Proxy3（DIRECT）：23131 → 4434 (TLS) → 3132  

※ 本構成はポート番号そのものを目的とせず、  
通信経路・責務・暗号化境界を明示的に分離するための設計としています。
