...
### ✅ 目的（最重要）
- 商用製品を含む複合構成では、障害解析がブラックボックス化しやすく、設計意図や原因を自分の言葉で説明しづらい場面がある 
...
> 一般的な企業ネットワークで求められる責務を題材に、
> **ローカル検証環境で再現・検証できる形へ分解して設計しています。
...
※本ポートフォリオは、学習・検証目的で作成したローカル検証環境の構成です。  
実在する企業・顧客・現場環境・設定値・ネットワーク構成を示すものではありません。  
記載している構成図・名称・経路・ログは、技術理解を説明するために一般化・抽象化したものです。
...
<details>
  ...
  <ul>
    <li><strong>位置づけ:</strong> 企業ネットワークで一般的に求められる責務を題材にした個人OSS開発（ポートフォリオ本体）</strong></li>
...
本プロジェクトは、企業ネットワークで一般的に求められる **多段プロキシ／認証基盤／ログ基盤／監視基盤、** を題材に、  
...
この章では、本プロジェクトの全体構成を **「一般化した商用系構成モデル」と「OSSで構築した検証構成」** で比較します。  
...
> **左：一般化モデル**  
> **右：OSS検証構成** … 入口/分岐/出口・認証・暗号化・検査・ログを分解した構成
...
<details>
  ...
  <ul>
    ...
    <li>
      <strong>エンタープライズ環境で求められる責務・振る舞い</strong>を持つ検証システムを、
...
本プロジェクトでは、**検証環境上での動作証跡（スクリーンショット／ログ）**を  
...
<ul>
  ...
  <li>認証・名前解決の前提条件を崩さず、 <strong>安定して再現</strong>できることを優先しました</li>
</ul>
...
<details>
  ...
  <ul>
    ...
    <li>
      ...
      認証・名前解決の前提を保った状態で、安定した検証が行えるようにしています
    </li>
  </ul>
...
### ネットワーク制御
<ul>
  <li>PAC による経路制御（通常/DIRECT）の設計と検証</li>
