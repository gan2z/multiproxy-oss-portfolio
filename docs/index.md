...
## このポートフォリオで伝えたいこと（30秒サマリ）

- 商用プロキシ環境の**構造・責務・制約**を理解するために
- OSS を用いて **入口／分岐／出口・認証・検査・ログを分解設計**
- 多段 SSLBump / Kerberos / WPAD / stunnel を **失敗込みで検証**
- 「設定できる」ではなく **「なぜそうなるか説明できる」状態を目指した**
- **設計→検証→ログでの根拠提示までを一貫して行い、運用目線で説明できる状態を作りました**
