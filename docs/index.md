...
本構成は、現場で運用されている**商用プロキシ環境の機能構成を参考にしつつ**、  
各機能・責務を分解し、**OSS を用いて再設計・実装したアーキテクチャ**です。

左図（参考）では商用プロキシ製品に内包されている機能を、  
右図では **入口／分岐／出口という役割単位**と  
**認証・検査・ログ・監視といった横断機能**に分離して構成しています。
...
<p class="note"><em>
※ 商用プロキシ環境では一体化されている  
復号・暗号化・認証・検査・ログ・監視といった機能を、  
本構成では OSS を用いて役割単位で分解・再構成しています。
</em></p>
